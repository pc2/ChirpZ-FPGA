...
void bluestein(float *in_re, float *in_im, const size_t n) {
    ...
    printf("Filter Before FFT\n");
    for (int i = 0; i < nb; i++){
      printf("%d: (%f , %f) \n", i, y_re[i], y_im[i]);
    }
    fft_pow_2(y_re, y_im, nb, -1);
    printf("Filter after FFT\n");
    for (int i = 0; i < nb; i++){
      printf("%d: (%f , %f) \n", i, y_re[i], y_im[i]);
    }
    printf("\n\n");
    printf("Chirp Data Creation\n");
    for (int i = 0; i < n; i++) {
      b_re[i] = w_re[i] * in_re[i] + w_im[i] * in_im[i];
      b_im[i] = w_re[i] * in_im[i] - w_im[i] * in_re[i];
      printf("%d: %f = %f * %f + %f * %f\n", i, b_re[i], w_re[i], in_re[i], w_im[i], in_im[i]);
      printf("%d: %f = %f * %f - %f * %f\n", i, b_im[i], w_re[i], in_im[i], w_im[i], in_re[i]);
      printf("\n");
    }
    printf("\n\n");
    printf("Signal Before FFT\n");
    for (int i = 0; i < nb; i++){
      printf("%d: (%f , %f) \n", i, b_re[i], b_im[i]);
    }
    fft_pow_2(b_re, b_im, nb, -1);
    printf("Signal after FFT\n");
    for (int i = 0; i < nb; i++){
      printf("%d: (%f , %f) \n", i, b_re[i], b_im[i]);
    }
    printf("\n\n");
    printf("Point wise Multiplication\n");
    for (int i = 0; i < nb; i++) {
...
int main(int argc, char **argv) {
    ...
    printf("Result from FFT\n");
    for (int i = 0; i < size; ++i) {
        printf("%d: (%f , %f) \n", i, test_re[i], test_im[i]);
    }
    printf("\n\n");
