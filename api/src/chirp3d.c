...
fpga_t fftfpgaf_c2c_chirp3d(const unsigned num, const float2 *inp, float2 *out, const bool inv, const unsigned batch){
  ...
  printf("\tNumber of points: %ux%ux%u\n\tNext Power of 2: %ux%ux%u\n\tChirp Power of 2: %ux%ux%u\n\tBatch: %u\n", num, num, num, next_num, next_num, next_num, chirp_num, chirp_num, chirp_num, batch);
  ...
  float2 *temp_in = malloc(sizeof(float2) * next_num * next_num * next_num * batch);
  float2 *temp_out = malloc(sizeof(float2) * next_num * next_num * next_num * batch);
  for(unsigned i = 0; i < batch * next_num * next_num * next_num; i++){
    temp_in[i].x = 0.0f;
    temp_in[i].y = 0.0f;
    temp_out[i].x = 0.0f;
    temp_out[i].y = 0.0f;
  ...
  // copy 31x31x31 buffer into a 32x32x32
  for(unsigned l = 0; l < batch; l++){ 
    for(unsigned i = 0; i < num; i++){
      for(unsigned j = 0; j < num; j++){   
        for(unsigned k = 0; k < num; k++){ 
          unsigned index_in = (l * num * num * num) + (i * num * num) + (j * num) + k;
          unsigned index_out = (l * next_num * next_num * next_num) + (i * next_num * next_num) + (j * next_num) + k;
  ...
  */
  ...
  cl_mem d_Signal = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_CHANNEL_3_INTELFPGA, sizeof(float2) * next_num * next_num * next_num * batch, NULL, &status);
  checkError(status, "Failed to allocate Signal buffer\n");

  cl_mem d_temp1 = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_CHANNEL_4_INTELFPGA, sizeof(float2) * next_num * next_num * next_num, NULL, &status);
  //checkError(status, "Failed to allocate temp1 buffer\n");

  //cl_mem d_temp2 = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_CHANNEL_4_INTELFPGA, sizeof(float2) * next_num * next_num * next_num, NULL, &status);
  //checkError(status, "Failed to allocate temp2 buffer\n");

  cl_mem d_Out = clCreateBuffer(context, CL_MEM_WRITE_ONLY | CL_CHANNEL_2_INTELFPGA, sizeof(float2) * next_num * next_num * next_num * batch, NULL, &status);
  ...
  */
  ...
  int inverse_sig = 1;
  unsigned ifft1d_1_count = batch * next_num * next_num;
  ...
  unsigned scale1_count = batch * next_num * next_num;
  ...
  unsigned demod1_count = batch * next_num * next_num;
  ...
  unsigned transpose1_count = batch * next_num;
  ...
  status = clSetKernelArg(kernel_transpose, 1, sizeof(cl_uint), (void *)&transpose1_count);
  checkError(status, "Failed to set kernel transpose arg 1"); 

  unsigned mod2_count = batch * next_num * next_num;
  ...
  unsigned fft1d_2_count = (batch * next_num * next_num);
  ...
  unsigned mult2_count = batch * next_num * next_num;
  // Filter is stored in BRAM, then multiplied with count * signals 
  ...
  unsigned scale2_count = batch * next_num * next_num;
  ...
  unsigned demod2_count = batch * next_num * next_num;
  ...
  unsigned mode = WR_GLOBALMEM;
  cl_kernel kernel_trans3D = clCreateKernel(program, "transpose3D", &status);
  status = clSetKernelArg(kernel_trans3D, 0, sizeof(cl_mem), (void *)&d_Out);
  checkError(status, "Failed to set kernel transpose3D arg 0");
  status = clSetKernelArg(kernel_trans3D, 1, sizeof(cl_mem), (void *)&d_temp1);
  checkError(status, "Failed to set kernel transpose3D arg 1");
  status = clSetKernelArg(kernel_trans3D, 2, sizeof(cl_uint), (void *)&num);
  checkError(status, "Failed to set kernel transpose3D arg 2"); 
  ...
  status = clSetKernelArg(kernel_mod3, 0, sizeof(cl_mem), (void *)&d_bufW);
  ...
  unsigned fft1d_3_count = (batch * next_num * next_num);
  cl_kernel kernel_fft1d_3 = clCreateKernel(program, "fft1d_3", &status);
  checkError(status, "Failed to create fft1d_3 kernel");
  status = clSetKernelArg(kernel_fft1d_3, 0, sizeof(cl_uint), (void *)&fft1d_3_count);
  checkError(status, "Failed to set kernel fft1d_3 arg 0"); 
  inverse = 0;
  status = clSetKernelArg(kernel_fft1d_3, 1, sizeof(cl_int), (void *)&inverse);
  checkError(status, "Failed to set kernel fft1d_3 arg 1"); 

  unsigned mult3_count = batch * next_num * next_num;
  // Filter is stored in BRAM, then multiplied with count * signals 
  ...
  cl_kernel kernel_ifft1d_3 = clCreateKernel(program, "ifft1d_3", &status);
  checkError(status, "Failed to create ifft1d_3 kernel");
  status = clSetKernelArg(kernel_ifft1d_3, 0, sizeof(cl_uint), (void *)&ifft1d_3_count);
  checkError(status, "Failed to set kernel ifft1d_3 arg 0"); 
  status = clSetKernelArg(kernel_ifft1d_3, 1, sizeof(cl_int), (void *)&inverse_sig);
  checkError(status, "Failed to set kernel ifft1d_3 arg 1");

  unsigned scale3_count = batch * next_num * next_num;
  cl_kernel kernel_scale3 = clCreateKernel(program, "scale3", &status);
  status = clSetKernelArg(kernel_scale3, 0, sizeof(cl_float), (void *)&factor);
  checkError(status, "Failed to set kernel scale3 arg 0"); 
  status = clSetKernelArg(kernel_scale3, 1, sizeof(cl_uint), (void *)&scale3_count);
  checkError(status, "Failed to set kernel scale3 arg 1"); 

  unsigned demod3_count = batch * next_num * next_num;
  cl_kernel kernel_demod3 = clCreateKernel(program, "demodulate3", &status);
  status = clSetKernelArg(kernel_demod3, 0, sizeof(cl_mem), (void *)&d_bufW);
  checkError(status, "Failed to set kernel demodulate3 arg 0");
  status = clSetKernelArg(kernel_demod3, 1, sizeof(cl_uint), (void *)&num);
  checkError(status, "Failed to set kernel demodulate3 arg 1"); 
  status = clSetKernelArg(kernel_demod3, 2, sizeof(cl_uint), (void *)&demod3_count);
  checkError(status, "Failed to set kernel demodulate3 arg 2"); 

  cl_kernel kernel_store = clCreateKernel(program, "store", &status);
  status = clSetKernelArg(kernel_store, 0, sizeof(cl_mem), (void *)&d_Out);
  checkError(status, "Failed to set kernel store arg 0");
  ...
  status = clEnqueueWriteBuffer(queue1, d_Signal, CL_FALSE, 0, sizeof(float2) * next_num * next_num * next_num * batch, temp_in, 0, NULL, &writeBuf_event);
  ...
  */
  modulate = MODULATE;
  unsigned fetch_count = batch * next_num * next_num;
  ...
  unsigned kernel_mod1_count = batch * next_num * next_num;
  status = clSetKernelArg(kernel_mod1, 1, sizeof(cl_uint), (void *)&modulate);
  checkError(status, "Failed to set kernel modulate1 arg 1"); 
  ...
  mode = RD_GLOBALMEM;
  status = clSetKernelArg(kernel_trans3D, 0, sizeof(cl_mem), (void *)&d_temp1);
  checkError(status, "Failed to set kernel transpose3D arg 0");
  status = clSetKernelArg(kernel_trans3D, 1, sizeof(cl_mem), (void *)&d_Out);
  checkError(status, "Failed to set kernel transpose3D arg 1");
  status = clSetKernelArg(kernel_trans3D, 2, sizeof(cl_uint), (void *)&num);
  checkError(status, "Failed to set kernel transpose3D arg 2"); 
  status = clSetKernelArg(kernel_trans3D, 3, sizeof(cl_uint), (void *)&mode);
  ...
  status = clEnqueueTask(queue9, kernel_store, 0, NULL, &stopSignal_event);
  checkError(status, "Failed to launch store kernel");
  status = clEnqueueTask(queue8, kernel_demod3, 0, NULL, NULL);
  checkError(status, "Failed to launch demod2 kernel");
  status = clEnqueueTask(queue7, kernel_scale3, 0, NULL, NULL);
  checkError(status, "Failed to launch scale2 kernel");
  ...
  status = clEnqueueReadBuffer(queue1, d_Out, CL_TRUE, 0, sizeof(float2) * next_num * next_num * next_num * batch, temp_out, 0, NULL, &readBuf_event);
  ...
  /*
  for(unsigned i = 0; i < next_num * next_num * next_num; i++){
    printf("Trans3D[%u]: (%f, %f)\n", i, temp_out[i].x, temp_out[i].y);
  }
  printf("\n");
  */
  ...
  for(unsigned l = 0; l < batch; l++){
    for(unsigned i = 0; i < num; i++){
      for(unsigned j = 0; j < num; j++){
        for(unsigned k = 0; k < num; k++){
          out[(l * num * num * num) + (i * num * num) + (j * num) + k].x = temp_out[(l * next_num * next_num * next_num) + (i * next_num * next_num) + (j * next_num) + k].x;

          out[(l * num * num * num) + (i * num * num) + (j * num) + k].y = temp_out[(l * next_num * next_num * next_num) + (i * next_num * next_num) + (j * next_num) + k].y;
        }
      }
    }
  }
  ...
  if(kernel_trans3D)
    clReleaseKernel(kernel_trans3D);
